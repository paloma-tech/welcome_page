  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #4A6CF7; padding: 20px; text-align: center;">
        <h1 style="color: white; margin: 0; font-size: 24px;">PalomaERP</h1>
      </div>
      ...
      <div style="background-color: #f5f5f5; padding: 15px; text-align: center; font-size: 12px; color: #777;">
        <p>&copy; ${new Date().getFullYear()} PalomaERP. All rights reserved.</p>
      </div>
    </div>
  `;
...
export const sendPasswordResetEmail = async (
  ...
  try {
    ...
    console.log('Password reset email sent successfully with Resend. ID:', data?.id);
    return data;
  } catch (error) {
    console.error('Exception when sending password reset email with Resend:', error);
    if (error instanceof Error) {
      console.error('Error message:', error.message);
      console.error('Error stack:', error.stack);
    } else {
